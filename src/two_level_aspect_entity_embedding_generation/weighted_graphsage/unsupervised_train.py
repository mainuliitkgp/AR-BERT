...
flags.DEFINE_string('train_prefix', './cluster_kg/community_graph_dbpedia_pagelink', 'name of the object file that stores the training data. must be specified.')
flags.DEFINE_string('weighted_adjacency_matrix_file', './cluster_kg/community_graph_dbpedia_pagelink-weighted_adjacency_matrix.npy', 'name of the object file that stores the weighted adjacency matrix. must be specified.')

# left to default values in main experiments 
flags.DEFINE_integer('epochs', 100, 'number of epochs to train.')
flags.DEFINE_float('dropout', 0.0, 'dropout rate (1 - keep probability).')
flags.DEFINE_float('weight_decay', 0.0, 'weight for l2 loss on embedding matrix.')
flags.DEFINE_integer('max_degree', 341, 'maximum node degree.')
...
def train(train_data, test_data=None):
    ...
    minibatch = EdgeMinibatchIterator(G, 
            id_map,
            placeholders, prob_matrix, batch_size=FLAGS.batch_size,
    ...
    if FLAGS.model == 'graphsage_mean':
        # Create model
        sampler = UniformNeighborSampler(adj_info, prob_matrix)
    ...
    for epoch in range(FLAGS.epochs): 
        ...
        print('epoch: '+str(epoch)+'\t'+'train_loss: '+str(epoch_train_loss)+'\n')
        fp.write('epoch: '+str(epoch)+'\t'+'train_loss: '+str(epoch_train_loss)+'\n')
        ...
        if epoch==0:
            ...
            sess.run(val_adj_info.op)
            node_embeddings, node_list = save_val_embeddings(sess, model, minibatch, FLAGS.validate_batch_size, log_dir(), layer_infos)
    ...
    for i, txt in enumerate(node_list):
        ...
        print(str(txt)+' '+str(tsne_df['tsne0'][i])+',' +str(tsne_df['tsne1'][i]))
...
def main(argv=None):
    ...
    train_data = load_data(FLAGS.train_prefix, FLAGS.weighted_adjacency_matrix_file, load_walks=True)
