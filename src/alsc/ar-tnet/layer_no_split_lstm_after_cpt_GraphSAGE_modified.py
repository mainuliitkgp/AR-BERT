...
from nn_utils_sentence_GraphSAGE_modified import *
import pickle
...
class LSTM:
    def __init__(self, n_in, n_out, name):
        ...
        # b shape: (4*n_out
        self.h0 = theano.shared(value=zeros(size=(n_out)), name='h0')
        self.c0 = theano.shared(value=zeros(size=(n_out)), name='c0')
...
class TNet:
    ...
    def __init__(self, args):
        ...
        self.model_dir = './models_no_split_lstm_after_cpt_GraphSAGE/'+self.ds_name+'/'
        ...
        self.LSTM_ctx = LSTM(n_in=self.n_in, n_out=self.n_rnn_out, name="CTX_LSTM")
        self.LSTM_tgt = LSTM(n_in=self.n_in, n_out=self.n_rnn_out, name="TGT_LSTM")
        self.LSTM_cpt = LSTM(n_in=((2*self.n_rnn_out)+self.graphsage_dim), n_out=self.n_rnn_out_after_cpt, name="CPT_LSTM")
        ...
        self.FC = Linear(n_in=((self.n_filters*len(self.kernels)) + (self.sent_len*2*self.n_rnn_out_after_cpt)), n_out=self.n_y, name="LAST_FC")
    ...
    def build_model(self):
        ...
        bs = T.shape(self.x)[0]
        input = self.Words[T.cast(self.x.flatten(), 'int32')].reshape((bs, self.sent_len, self.n_in))
        input_target = self.Words[T.cast(self.xt.flatten(), 'int32')].reshape((bs, self.target_len, self.n_in))
        ...
        H0_forward = self.LSTM_ctx(self.n_rnn_out, x=rnn_input)
        Ht_forward = self.LSTM_tgt(self.n_rnn_out, x=rnn_input_target)
        H0_backward = reverse_tensor(tensor=self.LSTM_ctx(self.n_rnn_out, x=rnn_input_reverse))
        Ht_backward = reverse_tensor(tensor=self.LSTM_tgt(self.n_rnn_out, x=rnn_input_target_reverse))
        ...
        H2_after_lstm_forward = self.LSTM_cpt(self.n_rnn_out, x=H2_concat)
        H2_after_lstm_backward = reverse_tensor(tensor=self.LSTM_cpt(self.n_rnn_out, x=reverse_tensor(tensor=H2_concat)))
        H2_after_lstm = T.concatenate([H2_after_lstm_forward, H2_after_lstm_backward], axis=2)

        #reshape to batch_size*(max_sequence_length*2*hidden_size_after_cpt)
        H2_after_lstm_shape = T.shape(H2_after_lstm)
        H2_after_lstm_reshape = T.reshape(
            H2_after_lstm, 
            (H2_after_lstm_shape[0], H2_after_lstm_shape[1]*H2_after_lstm_shape[2])
        ) 
        ...
        concat_hidden = T.concatenate([H2_after_lstm_reshape, feat], axis = 1)
