...
import time
import os
from layer_no_split_lstm_after_cpt_first_and_last_hidden_concat import TNet
from utils_no_split import *
...
if __name__ == '__main__':
    ...
    parser.add_argument("-did", type=int, default=0, help="gpu device id")
    ...
    for i in range(1, args.n_epoch + 1):
        ...
        for j in range(n_train_batches):
            ...
            train_x, train_xt, train_y, train_pw, sentence = get_batch_input(dataset=train_set, bs=args.bs, idx=j)

            y_pred, y_gold, loss, feature_maps_argmax = model.train(train_x, train_xt, train_y, train_pw, np.int32(1))
        ...
        for j in range(n_test_batches):
            ...
            test_x, test_xt, test_y, test_pw, sentence = get_batch_input(dataset=test_set, bs=args.bs, idx=j)

            y_pred, y_gold, loss, feature_maps_argmax = model.test(test_x, test_xt, test_y, test_pw, np.int32(0))
    ...
    plt.ylabel('loss')
    plt.legend()
    plt.savefig('loss_trend_'+cur_model_name+'_'+args.ds_name+'_'+str(args.n_epoch)+'_'+str(args.lr)+'_no_split_lstm_after_cpt_first_and_last_hidden_concat.png')
    plt.clf()
    ...
    plt.ylabel('acc')
    plt.legend()
    plt.savefig('acc_trend_'+cur_model_name+'_'+args.ds_name+'_'+str(args.n_epoch)+'_'+str(args.lr)+'_no_split_lstm_after_cpt_first_and_last_hidden_concat.png')
    plt.clf()
    ...
    result_logs.append('-------------------------------------------------------\n')
    if not os.path.exists('./log_no_split_lstm_after_cpt_first_and_last_hidden_concat'):
        os.makedirs('log_no_split_lstm_after_cpt_first_and_last_hidden_concat')
    with open("./log_no_split_lstm_after_cpt_first_and_last_hidden_concat/%s_%s_%s_%s.txt" % (cur_model_name, args.ds_name, str(args.n_epoch), str(args.lr)), 'w') as fp:
        fp.writelines(result_logs)

    with open("./log_no_split_lstm_after_cpt_first_and_last_hidden_concat/%s_%s_%s_%s_visualize_train.txt" % (cur_model_name, args.ds_name, str(args.n_epoch), str(args.lr)), 'w') as fp_train:
    ...
    with open("./log_no_split_lstm_after_cpt_first_and_last_hidden_concat/%s_%s_%s_%s_visualize_train_wrong_prediction_only.txt" % (cur_model_name, args.ds_name, str(args.n_epoch), str(args.lr)), 'w') as fp_train:
    ...
    with open("./log_no_split_lstm_after_cpt_first_and_last_hidden_concat/%s_%s_%s_%s_visualize_test.txt" % (cur_model_name, args.ds_name, str(args.n_epoch), str(args.lr)), 'w') as fp_test:
    ...
    with open("./log_no_split_lstm_after_cpt_first_and_last_hidden_concat/%s_%s_%s_%s_visualize_test_wrong_prediction_only.txt" % (cur_model_name, args.ds_name, str(args.n_epoch), str(args.lr)), 'w') as fp_test:
    ...
    np.save("./log_no_split_lstm_after_cpt_first_and_last_hidden_concat/%s_%s_%s_%s_correct_test_label.npy" % (cur_model_name, args.ds_name, str(args.n_epoch), str(args.lr)), correct_label)
    np.save("./log_no_split_lstm_after_cpt_first_and_last_hidden_concat/%s_%s_%s_%s_predicted_test_label_original.npy" % (cur_model_name, args.ds_name, str(args.n_epoch), str(args.lr)), predicted_label)
