...
def read(path, wikified_file_path, extracted_target_file_path, graphsage_indices_file_path, graphsage_file_path):
        """
        """
        wikified_target = []
        with open(wikified_file_path) as w_fp:
            for line in w_fp:
                line = line.strip()
                if(line.startswith('http://en.wikipedia.org/wiki/')):
                    line=line[len('http://en.wikipedia.org/wiki/'):]
                wikified_target.append(line)

        extracted_target = []
        with open(extracted_target_file_path) as et_fp:
            for line in et_fp:
                extracted_target.append(line.strip())
        
        graphsage_indices = []
        with open(graphsage_indices_file_path) as gi_fp:
            for line in gi_fp:
                graphsage_indices.append(line.strip())
        
        graphsage_embedding = np.load(graphsage_file_path)
        ...
        with open(path) as fp:
            for counter, line in enumerate(fp):
                record = {}
                index_of_wikified_target_in_extracted_target = extracted_target.index(wikified_target[counter])
                index_of_wikified_target_in_graphsage = graphsage_indices.index(str(index_of_wikified_target_in_extracted_target))
                ...
                record['graphsage_embedding'] = graphsage_embedding[index_of_wikified_target_in_graphsage]
...
def load_data(connection_type, ds_name):
        ...
        train_wikified_file='./dataset/%s/%s_train_wikipedia_target.txt' % (ds_name, ds_name)
        test_wikified_file='./dataset/%s/%s_test_wikipedia_target.txt' % (ds_name, ds_name)
        extracted_target_file = './dataset/%s/%s_extracted_targets.txt' % (ds_name, ds_name)
        graphsage_indices_file = './dataset/%s/val.txt' % ds_name
        graphsage_file = './dataset/%s/val.npy' % ds_name
        train_set = read(path=train_file, wikified_file_path=train_wikified_file, extracted_target_file_path=extracted_target_file, graphsage_indices_file_path=graphsage_indices_file, graphsage_file_path = graphsage_file)
        test_set = read(path=test_file, wikified_file_path=test_wikified_file, extracted_target_file_path=extracted_target_file, graphsage_indices_file_path=graphsage_indices_file, graphsage_file_path = graphsage_file)
